...
use super::sync::cell::UnsafeCell;
...
impl<T> Mutex<T> {
    ...
    fn lock_slow(&self) -> MutexGuard<'_, T> {
        ...
        loop {
            ...
            spins_left = match spins_left.checked_sub(1) {
                Some(spins_left) => spins_left,
                None => {
                    // SAFETY: This is unsound lmao
                    let dp = unsafe { arduino_hal::Peripherals::steal() };
                    let pins = arduino_hal::pins!(dp);
                    let mut serial = arduino_hal::default_serial!(dp, pins, 57600);

                    ufmt::uwriteln!(&mut serial, "Spinning for too long..").void_unwrap();

                    panic!("Exhausted the spin limit.")
                }
            };
